...
uint32_t core_decode_execute_complete(core_state *basic_core, uint8_t *mem) {
    uint32_t ir, *msr, *ip, *lr, *usp, *psp, *pidr, *gpr_file;
    uint32_t opcode, regA, regB, regC, imm12, imm4, imm2, imm1, ret, link, priv_to_user; 
    ...
    switch (opcode) {
        ...
        case BR:
            ...
            branch(condition_code, addr, link, ret, priv_to_user, msr, ip, lr, usp, pidr, mem, gpr_file);
            break;

        case SYS_CALL:
            sys_call(imm4, msr, ip, lr, usp, pidr, mem, gpr_file);
            break;
